...
def search_sign(file_out: str, sign: str = "'", ldap_base_dn: str = LDAP_BASE_DN) -> bool:
    pattern = r'[A-Z|0-9|_]*\'[A-Z|0-9|_]*'
    search_filter = f"(cn={pattern})"
    with open(f'/home/project/AD_INTEGRATION/data/{file_out}', 'w+', encoding='UTF-8') as new_file:
        with ldap_conn() as c:
            c.search(search_base=ldap_base_dn,
                     search_filter=search_filter,
