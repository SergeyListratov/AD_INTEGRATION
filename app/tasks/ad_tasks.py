...
def create_i_ad_user() -> list[dict[str, Any]]:
    result_list = list()
    i_usr_list = find_group_member_for_internet()
    if i_usr_list:
        for j in range(len(i_usr_list)):
            result_dict = create_i_ad_one_user(i_usr_list[j])
            result_list.append(result_dict)

    return result_list


def create_i_ad_one_user(usr: dict) -> dict[str, Any]:
    with i_ldap_conn() as conn:
        empty_fild_list = []
        inet_dao = InetDao()
        if 'givenName' in usr['attributes']:
            inet_dao.data['first_name'] = usr['attributes']['givenName']
        else:
            inet_dao.data['first_name'] = ''
            empty_fild_list.append('Имя')
        if 'sn' in usr['attributes']:
            inet_dao.data['last_name'] = usr['attributes']['sn']
        else:
            inet_dao.data['last_name'] = ''
            empty_fild_list.append('Фамилия')
        if 'department' in usr['attributes']:
            inet_dao.data['division'] = usr['attributes']['department']
        else:
            inet_dao.data['division'] = ''
            empty_fild_list.append('Подразделение')
        if 'title' in usr['attributes']:
            inet_dao.data['role'] = usr['attributes']['title']
        else:
            inet_dao.data['role'] = ''
            empty_fild_list.append('Должность')
        if 'initials' in usr['attributes']:
            inet_dao.data['number'] = usr['attributes']['initials']
        else:
            inet_dao.data['number'] = ''
            empty_fild_list.append('Табельный')
        if 'sAMAccountName' in usr['attributes']:
            inet_dao.data['login_name'] = usr['attributes']['sAMAccountName']
        else:
            inet_dao.data['login_name'] = ''
        ...
        if empty_fild:
            ...
            inet_dao.data['status'], inet_dao.data['message'], inet_dao.data['i_password'] = 'ERROR', msg, 'ERROR'
            result_dict = {'status': inet_dao.data['status'], 'message': inet_dao.data['message'],
                           'i_password': inet_dao.data['i_password']}
            ...
            if not result:
                ...
                inet_dao.data['status'], inet_dao.data['message'], inet_dao.data['i_password'] = 'OK', msg, new_pass
                result_dict = {'status': inet_dao.data['status'], 'message': inet_dao.data['message'],
                               'i_password': inet_dao.data['i_password'], 'login_name': inet_dao.data['login_name']}
